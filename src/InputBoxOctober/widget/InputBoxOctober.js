...
define([
    ...
    return declare("InputBoxOctober.widget.InputBoxOctober", [_WidgetBase, _TemplatedMixin], {
        ...
        _setMaskText : function(obj) {
          dojoProp.set(this.inputBox, "value", "")
          if (obj != null) {
            if (obj.get(this.name) == "") {
              dojoProp.set(this.inputBox, "placeholder", this.maskString);
            } else {
          ...
          if (this.customPH) {
            $(this.inputBox).mask(this.maskString, {placeholder: this.customPH});
          } else {
            $(this.inputBox).mask(this.maskString);
          }
        }
